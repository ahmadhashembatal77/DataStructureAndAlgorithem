...
private fun <T: Comparable<T>> merge(
    ...
    while (leftIndex < left.size && rightIndex < right.size)
    {
        ...
        if(leftElement < rightElement){result.add(leftElement); leftIndex+=1}
        else if(leftElement > rightElement){result.add(rightElement); rightIndex+=1}
        else{result.add(leftElement); leftIndex+=1; result.add(rightElement); rightIndex+=1}
    }
